...
- Управление лицензиями (создание, редактирование, удаление)
- Загрузка результатов турниров

**\*Ограничение**: MANAGER может удалять только игроков, которые **НЕ участвовали** в турнирах. Это правило сохраняет целостность истории соревнований.
...
5. **Защита от дублирования**: Middleware аудита автоматически предотвращает создание дубликатов записей для одного HTTP-запроса, даже если middleware вызывается несколько раз (например, из-за React StrictMode в режиме разработки).
